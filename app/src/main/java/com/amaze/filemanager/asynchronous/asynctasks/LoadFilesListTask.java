...
public class LoadFilesListTask
  ...
  private final boolean rootListing;
  ...
  protected void onProgressUpdate(Throwable... values) {
    for (Throwable exception : values) {
      if (openmode == OpenMode.SMB || exception instanceof SmbException) {
        showSmbError(exception);
      }
    }
  }

  /** Show one understandable SMB error instead of a raw jcifs exception/toast. */
  private void showSmbError(Throwable exception) {
    Context currentContext = context.get();
    MainFragment mainFragment = mainFragmentReference.get();
    if (currentContext == null) return;

    String rawMessage = exception.getMessage() == null ? "" : exception.getMessage().trim();
    String lower = rawMessage.toLowerCase(java.util.Locale.ROOT);
    final int messageRes;

    if (exception instanceof SmbAuthException
        || lower.contains("logon failure")
        || lower.contains("authentication")
        || lower.contains("wrong password")
        || lower.contains("access denied")) {
      messageRes = R.string.smb_error_auth;
      ...
      messageRes = R.string.smb_error_unreachable;
    } else if (lower.contains("bad network name")
        || lower.contains("object_name_not_found")
        || lower.contains("share not found")
        || lower.contains("does not exist")) {
      messageRes = R.string.smb_error_share;
    } else if ("/".equals(Uri.parse(path).getPath())) {
      messageRes = R.string.smb_error_root_listing;
    } else {
      messageRes = R.string.smb_error_generic;
    }

    String message = currentContext.getString(messageRes);
    if (!rawMessage.isEmpty()) {
      message += "\n\n" + currentContext.getString(R.string.smb_error_technical_details, rawMessage);
    }

    MaterialAlertDialogBuilder builder =
        new MaterialAlertDialogBuilder(currentContext)
            .setTitle(R.string.smb_error_title)
            .setMessage(message)
            .setPositiveButton(android.R.string.ok, null);
    if (mainFragment != null) {
      builder.setNegativeButton(
          R.string.smb_edit_connection,
          (dialog, which) -> mainFragment.reauthenticateSmb());
    }
    builder.show();
  }
  ...
  private List<LayoutElementParcelable> listDefault(
    ...
    if (localCache != null && !forceReload) {
      ...
      if (OpenMode.ROOT.equals(openmode)
          && list.isEmpty()
          && localCache != null
          && !localCache.isEmpty()) {
        // Root shells can transiently return an empty command result while Magisk/libsu is being
        // reattached. Keep the last known-good directory contents instead of replacing the UI with
        // an empty list. A later refresh can then update it normally.
        LOG.warn("Root refresh for [{}] returned no entries; keeping cached listing", path);
        list.clear();
        list.addAll(localCache);
      }
      // Root directories are usually small, so the generic >100-item cache threshold meant `/`
      // often had no last-known-good list to restore after a transient libsu failure. Cache every
      // successful root listing; retain the threshold for ordinary local directories.
      if (!list.isEmpty()
          && (OpenMode.ROOT.equals(openmode)
              || list.size() > MainActivityViewModel.Companion.getCACHE_LOCAL_LIST_THRESHOLD())) {
        mainActivityViewModel.putInCache(path, list);
      }
    }
    return list;
