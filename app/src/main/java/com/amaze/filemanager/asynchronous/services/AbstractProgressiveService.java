...
public abstract class AbstractProgressiveService extends Service
  ...
  /**
   * Android 12+ does not automatically apply NotificationCompat.Builder#setColor to ProgressBar
   * instances inside custom RemoteViews. Tint both determinate and indeterminate states explicitly
   * so foreground-operation progress uses the same readable Amaze/Monet accent as its action text.
   */
  protected final void applyNotificationProgressTint(@ColorInt int accentColor) {
    if (SDK_INT < S) return;

    final ColorStateList tint = ColorStateList.valueOf(accentColor);
    final RemoteViews small = getNotificationCustomViewSmall();
    final RemoteViews big = getNotificationCustomViewBig();
    small.setColorStateList(
        R.id.notification_service_progressBar_small, "setProgressTintList", tint);
    small.setColorStateList(
        R.id.notification_service_progressBar_small, "setIndeterminateTintList", tint);
    big.setColorStateList(R.id.notification_service_progressBar_big, "setProgressTintList", tint);
    big.setColorStateList(
        R.id.notification_service_progressBar_big, "setIndeterminateTintList", tint);
  }
