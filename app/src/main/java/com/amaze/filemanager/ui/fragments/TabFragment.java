...
public class TabFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    if (savedInstanceState == null) {
      ...
      fragments.clear();
      try {
        fragments.add(restoreOrCreateFragment(savedInstanceState, KEY_FRAGMENT_0, 0));
        fragments.add(restoreOrCreateFragment(savedInstanceState, KEY_FRAGMENT_1, 1));
      } catch (Exception e) {
        LOG.warn("Failed to restore tab fragments; rebuilding both tabs", e);
        fragments.clear();
        fragments.add(createFallbackMainFragment(0));
        fragments.add(createFallbackMainFragment(1));
      }
  ...
  private class ScreenSlidePagerAdapter extends FragmentStateAdapter {
    ...
    public Fragment createFragment(int position) {
      Fragment fragment =
          position >= 0 && position < fragments.size() ? fragments.get(position) : null;
      if (fragment == null) {
        LOG.warn("Pager requested missing fragment at position {}; rebuilding it", position);
        fragment = createFallbackMainFragment(position);
        while (fragments.size() < position) {
          fragments.add(createFallbackMainFragment(fragments.size()));
        }
  ...
  private MainFragment createMainFragment(
      @NonNull Tab tab, String path, boolean hideFabInTab) {
  ...
  private MainFragment createFallbackMainFragment(int position) {
    final int tabNumber = position + 1;
    Tab tab = TabHandler.getInstance().findTab(tabNumber);
    if (tab == null) {
      String fallbackPath =
          position == 0
              ? requireMainActivity().getDrawer().getFirstPath()
              : requireMainActivity().getDrawer().getSecondPath();
      if (Utils.isNullOrEmpty(fallbackPath)) {
        fallbackPath = "/";
      }
      LOG.warn("Rebuilding missing tab {} at [{}]", tabNumber, fallbackPath);
      tab = new Tab(tabNumber, fallbackPath, fallbackPath);
    }
    return createMainFragment(tab, "", false);
  }

  private Fragment restoreOrCreateFragment(
      @NonNull Bundle savedInstanceState, @NonNull String key, int position) {
    Fragment restored = fragmentManager.getFragment(savedInstanceState, key);
    if (restored instanceof MainFragment) {
      return restored;
    }
    LOG.warn("Saved fragment {} was null; rebuilding tab {}", key, position + 1);
    return createFallbackMainFragment(position);
  }

  private void addTab(@NonNull Tab tab, String path, boolean hideFabInTab) {
    fragments.add(createMainFragment(tab, path, hideFabInTab));
    sectionsPagerAdapter.notifyDataSetChanged();
          ...
          new TabFragmentSideDragListener(
              () -> {
                if (viewPager.getCurrentItem() == 1) {
                  if (mainFragment != null && mainFragment.adapter != null) {
