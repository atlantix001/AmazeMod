  ...
  public void initStatusBarResources(View parentView) {
    ...
    if (SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      if (MonetColorHelper.isEnabled(this, getPrefs())) {
        // The legacy drawer path used FLAG_TRANSLUCENT_STATUS, which lets DrawerLayout draw a
        // dark compatibility scrim through the status-bar/cutout area. Material You uses an
        // opaque tonal status bar instead: no artificial shadow and no drawer content behind the
        // camera cutout.
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        // Android 15 enforces edge-to-edge for targetSdk 35. Let the app bar paint behind the
        // status bar instead of leaving the window background/scrim visible above it.
      ...
      } else if (findViewById(R.id.tab_frame) != null || findViewById(R.id.drawer_layout) == null) {
  ...
  void setTheme() {
    ...
    if (MonetColorHelper.isEnabled(this, getPrefs())) {
      if (theme.equals(AppTheme.LIGHT)) {
        setTheme(R.style.appCompatMonetLight);
      } else if (theme.equals(AppTheme.BLACK)) {
        setTheme(R.style.appCompatMonetBlack);
      } else {
        setTheme(R.style.appCompatMonetDark);
      }
      return;
    }
