...
public class MainFragment extends Fragment
  ...
  public void loadlist(
    ...
    boolean rootExplorerEnabled = getMainActivity() != null && getMainActivity().isRootExplorer();
    boolean requestedRootListing =
        rootExplorerEnabled && FileUtils.isRunningAboveStorage(actualPath);

    if (loadFilesListTask != null && loadFilesListTask.getStatus() != AsyncTask.Status.FINISHED) {
      String runningPath = loadFilesListTask.getPath();
      boolean runningRootListing = loadFilesListTask.isRootListing();

      if (requestedRootListing
          && runningRootListing
          && !loadFilesListTask.isCancelled()
          && TextUtils.equals(runningPath, actualPath)) {
        // Activity/fragment restoration can request the same root path several times within the
        // same frame. Interrupting the already-running libsu job is exactly what produced the
        // empty-root state: libsu consumes InterruptedException internally and returns an empty
        // result, which looks indistinguishable from a real empty directory to the parser. The
        // in-flight task is already loading the requested path, so keep it instead of replacing it.
        LOG.info("Root list task for [{}] already running; keeping in-flight request", actualPath);
        return;
      }

      if (runningRootListing) {
        // Do not interrupt a libsu shell job. Mark the obsolete task cancelled so its result is not
        // delivered, but allow the shell command itself to finish. Root list tasks use the serial
        // executor below, therefore a replacement starts only after the old shell job has left the
        // shared root shell in a consistent state.
        LOG.warn("Replacing root list task for [{}] without interrupting libsu", runningPath);
        loadFilesListTask.cancel(false);
      } else {
    ...
    loadFilesListTask.executeOnExecutor(
        requestedRootListing ? AsyncTask.SERIAL_EXECUTOR : AsyncTask.THREAD_POOL_EXECUTOR);
  }
  ...
  public void initTopAndEmptyAreaDragListeners(boolean destroy) {
    if (destroy) {
      ...
      if (adapter == null) {
        // During tab/lifecycle transitions the inactive fragment can exist before its RecyclerView
        // adapter is attached. Do not construct RecyclerAdapterDragListener with a null adapter.
        LOG.warn("Skipping drag listeners until RecyclerView adapter is available");
        listView.setOnDragListener(null);
        nofilesview.setOnDragListener(null);
        return;
      }
