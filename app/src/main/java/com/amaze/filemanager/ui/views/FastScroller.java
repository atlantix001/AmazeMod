...
public class FastScroller extends FrameLayout {
  ...
  private float computeHandlePosition() {
    ...
    if (currentRecyclerView == null || handle == null || columns <= 0) return -1;

    RecyclerView.Adapter<?> adapter = currentRecyclerView.getAdapter();
    if (adapter == null || adapter.getItemCount() == 0 || currentRecyclerView.getChildCount() == 0) {
      return -1;
    }

    View firstVisibleView = currentRecyclerView.getChildAt(0);
    int firstVisibleHeight = firstVisibleView == null ? 0 : firstVisibleView.getHeight();
    int viewportHeight = getHeightMinusPadding();
    if (firstVisibleView == null || firstVisibleHeight <= 0 || viewportHeight <= 0) return -1;

    float recyclerViewOversize =
        (float) firstVisibleHeight / columns * adapter.getItemCount() - viewportHeight;
    if (recyclerViewOversize <= 0) return -1;

    int recyclerViewAbsoluteScroll =
        currentRecyclerView.getChildLayoutPosition(firstVisibleView)
                / columns
                * firstVisibleHeight
  ...
  private boolean isRecyclerViewScrollable() {
    if (recyclerView == null || recyclerView.getAdapter() == null || columns <= 0) return false;
    View firstVisibleView = recyclerView.getChildAt(0);
    if (firstVisibleView == null || firstVisibleView.getHeight() <= 0) return false;
    return firstVisibleView.getHeight() * recyclerView.getAdapter().getItemCount() / columns
  ...
  void updateHandlePosition() {
    float relativePosition = computeHandlePosition();
    if (relativePosition >= 0
        && !Float.isNaN(relativePosition)
        && !Float.isInfinite(relativePosition)) {
      setHandlePosition1(relativePosition);
    }
  }
