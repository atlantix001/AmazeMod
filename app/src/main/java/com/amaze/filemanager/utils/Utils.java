...
public class Utils {
  ...
  public static boolean showFeedbackSnackbar(
      MainActivity mainActivity, CharSequence text, int length) {
    View anchor = mainActivity.findViewById(R.id.content_frame);
    if (anchor == null) {
      anchor = mainActivity.findViewById(android.R.id.content);
    }
    if (anchor == null) {
      return false;
    }
    Snackbar.make(anchor, text, length).show();
    return true;
  }
  ...
  public static Snackbar showCutCopySnackBar(
    ...
    final int snackBackground = mainActivity.getAccent();
    final int snackContent =
        ColorUtils.calculateContrast(Color.BLACK, snackBackground)
                >= ColorUtils.calculateContrast(Color.WHITE, snackBackground)
            ? Color.BLACK
            : Color.WHITE;
    ((CardView) snackBarLayout.findViewById(R.id.snackBarCardView))
        .setCardBackgroundColor(snackBackground);
    textView.setTextColor(snackContent);
    actionButton.setTextColor(snackContent);
    cancelButton.setTextColor(snackContent);
