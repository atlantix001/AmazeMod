...
public class AppConfig extends GlideApplication {
  ...
  public static void toast(Context context, @StringRes int message) {
    ...
    final Context toastContext = resolveToastContext(context);
    final @StringRes int m = message;
    getInstance()
        .runInApplicationThread(
            () -> {
              if (!(toastContext instanceof MainActivity)
                  || !Utils.showFeedbackSnackbar(
                      (MainActivity) toastContext,
                      ((MainActivity) toastContext).getString(m),
                      Snackbar.LENGTH_LONG)) {
                Toast.makeText(toastContext, m, Toast.LENGTH_LONG).show();
              }
            });
  }
    ...
    getInstance()
        .runInApplicationThread(
            () -> {
              if (!(toastContext instanceof MainActivity)
                  || !Utils.showFeedbackSnackbar(
                      (MainActivity) toastContext, m, Snackbar.LENGTH_LONG)) {
                Toast.makeText(toastContext, m, Toast.LENGTH_LONG).show();
              }
            });
  }
  ...
  /**
   * Keep transient messages on the currently visible activity theme whenever possible. The old
   * implementation always downgraded to the application context, which made otherwise identical
   * operation messages switch between light and dark system toast styling.
   */
  private static Context resolveToastContext(@NonNull Context context) {
    if (context instanceof Activity) {
      Activity activity = (Activity) context;
      if (!activity.isFinishing() && !activity.isDestroyed()) return activity;
    }

    Context mainContext = getInstance().getMainActivityContext();
  ...
  public Context getMainActivityContext() {
    return mainActivityContext != null ? mainActivityContext.get() : null;
  }
