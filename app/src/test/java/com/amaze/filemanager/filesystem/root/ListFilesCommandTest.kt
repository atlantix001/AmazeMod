...
class ListFilesCommandTest {
    ...
    fun testCommandRun() {
        ...
        // Filesystem root keeps the modern stat parser. Its command now resets cwd in the same
        // libsu job, so the relative root glob cannot observe stale shared-shell state.
    ...
    @Test
    fun testRootStatEmptyFallsBackToLegacy() {
        sharedPreferences.edit()
            .putBoolean(PreferencesConstants.PREFERENCE_ROOT_LEGACY_LISTING, false).commit()
        every {
            ListFilesCommand.runShellCommandToList(
        ...
        ListFilesCommand.listFiles(
            ...
            showHidden = false,
            openModeCallback = {},
            onFileFoundCallback = { ++count },
        )

        assertEquals(lsRootLines.size - 1, count)
    }

    @Test
    fun testCancelledRootStatDoesNotLaunchLegacyFallback() {
        sharedPreferences.edit()
            .putBoolean(PreferencesConstants.PREFERENCE_ROOT_LEGACY_LISTING, false).commit()
        every {
            ListFilesCommand.runShellCommandToList(
        ...
        } throws InterruptedIOException("Intentional cancellation")
